...
namespace ME3TweaksCore.Services.Backup
{
    ...
    public class BackupService
    {
        ...
        protected static bool SetProperty<T>(ref T field, T value, [CallerMemberName] string propertyName = "")
        {
        ...
        public static void RefreshBackupStatus(List<GameTarget> targets, MEGame game = MEGame.Unknown)
        {
            if (game is MEGame.ME1 or MEGame.Unknown)
            {
                RefreshBackupStatus(MEGame.ME1, targets == null || targets.Any(x => x.Game == MEGame.ME1),
                    ME1BackedUp, msg => ME1BackupStatus = msg, msg => ME1BackupStatusTooltip = msg);
            }

            if (game is MEGame.ME2 or MEGame.Unknown)
            {
                RefreshBackupStatus(MEGame.ME2, targets == null || targets.Any(x => x.Game == MEGame.ME2),
                    ME2BackedUp, msg => ME2BackupStatus = msg, msg => ME2BackupStatusTooltip = msg);
            }
            if (game is MEGame.ME3 or MEGame.Unknown)
            {
                RefreshBackupStatus(MEGame.ME3, targets == null || targets.Any(x => x.Game == MEGame.ME3), ME3BackedUp,
                    msg => ME3BackupStatus = msg, msg => ME3BackupStatusTooltip = msg);
            }
            if (game is MEGame.LE1 or MEGame.Unknown)
            {
                RefreshBackupStatus(MEGame.LE1, targets == null || targets.Any(x => x.Game == MEGame.LE1), LE1BackedUp,
                    msg => LE1BackupStatus = msg, msg => LE1BackupStatusTooltip = msg);
            }
            if (game is MEGame.LE2 or MEGame.Unknown)
            {
                RefreshBackupStatus(MEGame.LE2, targets == null || targets.Any(x => x.Game == MEGame.LE2), LE2BackedUp,
                    msg => LE2BackupStatus = msg, msg => LE2BackupStatusTooltip = msg);
            }
            if (game is MEGame.LE3 or MEGame.Unknown)
            {
                RefreshBackupStatus(MEGame.LE3, targets == null || targets.Any(x => x.Game == MEGame.LE3), LE3BackedUp,
                    msg => LE3BackupStatus = msg, msg => LE3BackupStatusTooltip = msg);
            }
            if (game is MEGame.LELauncher or MEGame.Unknown)
            {
                RefreshBackupStatus(MEGame.LELauncher, targets == null || targets.Any(x => x.Game == MEGame.LELauncher), LELBackedUp,
        ...
        private static void RefreshBackupStatus(MEGame game, bool installed, bool backedUp, Action<string> setStatus, Action<string> setStatusToolTip)
        {
            ...
            if (backedUp)
            {
                setStatus(LC.GetString(LC.string_backedUp));
                setStatusToolTip(LC.GetString(LC.string_interp_backupStatusStoredAt, bPath));
            }
            else if (bPath == null)
            {

                setStatus(LC.GetString(LC.string_notBackedUp));
                setStatusToolTip(LC.GetString(LC.string_gameHasNotBeenBackedUp));
            }
            else if (!Directory.Exists(bPath))
            {
                setStatus(LC.GetString(LC.string_backupUnavailable));
                setStatusToolTip(LC.GetString(LC.string_interp_backupStatusNotAccessible, bPath));
            }
            else
            {
                ...
                {
                    setStatus(LC.GetString(LC.string_backupNotVanilla));
                    setStatusToolTip(LC.GetString(LC.string_interp_backupStatusNotVanilla, nonVanillaPath));
                }
                else if (!installed)
                {
                    setStatus(LC.GetString(LC.string_notInstalled));
                    setStatusToolTip(LC.GetString(LC.string_gameNotInstalledHasItBeenRunOnce));
                }
                else
                {
        ...
        public static string GetGameBackupPath(MEGame game, bool forceCmmVanilla = true, bool logReturnedPath = false, bool forceReturnPath = false, bool refresh = false)
        {
            ...
            string path = MSharedSettings.GetSettingString($@"{game}VanillaBackupLocation");
        ...
        public static void MigrateBackupPaths()
        {
            ...
            {
                var storedPath = RegistryHandler.GetRegistryString(REGISTRY_KEY_ALOT, $@"ME1VanillaBackupLocation");
                if (storedPath != null)
                {
            ...
            {
                var storedPath = RegistryHandler.GetRegistryString(REGISTRY_KEY_ALOT, $@"ME2VanillaBackupLocation");
                if (storedPath != null)
                {
            ...
            {
                var storedPath = RegistryHandler.GetRegistryString(REGISTRY_KEY_ME3CMM, $@"VanillaCopyLocation");
                if (storedPath != null)
                {
